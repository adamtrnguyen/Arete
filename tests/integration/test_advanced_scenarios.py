...
def test_sync_prune_flow(tmp_path, anki_url, setup_anki, test_deck, run_arete):
    """Prune deletes the note whose file is gone and keeps the other."""

    def note(front: str) -> str:
        return (
            f"---\ndeck: {test_deck}\narete: true\ncards:\n"
            f"  - model: Basic\n    Front: {front}\n    Back: x\n---\n"
        )

    def find(front: str) -> list[int]:
        query = f'"deck:{test_deck}" "{front}"'
        return requests.post(
            anki_url, json={"action": "findNotes", "version": 6, "params": {"query": query}}
        ).json()["result"]

    (tmp_path / "prune_me.md").write_text(note("Prune Candidate"), encoding="utf-8")
    (tmp_path / "stay.md").write_text(note("Persistent Note"), encoding="utf-8")
    assert run_arete(tmp_path, anki_url).returncode == 0
    assert find("Prune Candidate") and find("Persistent Note")

    (tmp_path / "prune_me.md").unlink()
    res = run_arete(tmp_path, anki_url, args=["--prune", "--force"])
    assert res.returncode == 0, res.stderr

    assert find("Prune Candidate") == [], "Orphan note was not pruned"
    assert find("Persistent Note"), "A note still in the vault was pruned"
